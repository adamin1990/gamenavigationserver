...
<body>
	<div class="main2">
		...
		<s:form action="findAllStore" method="post" id="myframe" name="formName">
			<s:hidden name="page"/>
			<table>
				<tr>
				...
				<tr style="background-color: #4876FF">
					<td align="left" width="350px;" height="30px;">
					<input type="checkbox" name="alls" onClick="selectAll('formName','no')" title="全选/取消全选">应用商店名称</td>
					<td align="left" width="100px;" height="30px;">应用商店网址</td>
				</tr>
				<s:iterator value="pageBean.list" var="l">
					<tr onclick="selected(this)"
						...
						<td align="left" width="100px;" height="30px;">
						<a href="<s:property value="#this.url" />"><s:property value="#this.url" /></a></td>						
					</tr>
				</s:iterator>
			</table>
		...
		<script type="text/javascript">
			...
			function removet(){ 
				...
				formName.action="deleteStore.action"; 
				formName.submit();  
				} 
			function release(){
				window.location.href="<%=path%>/admin/addStore.action";
				}
			function modify(){ 
				if(selectedId==null)
				{
				...
				window.location.href="<%=path%>/admin/findStoreById.action?sid="+selectedId;
				} 
			/* 是否全选标记 */

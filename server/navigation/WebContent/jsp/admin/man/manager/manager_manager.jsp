...
<body>
	<div class="main2">
		...
		<s:form action="findAllManager" method="post" id="myframe" name="formName">
			<s:hidden name="page"/>
			<table>
				<tr> 
				...
				<tr style="background-color: #4876FF">
					...
					<input type="checkbox" name="alls" onClick="selectAll('formName','no')" title="全选/取消全选">用户名称</td>
					<td align="left" width="100px;" height="30px;">用户密码</td>
					<td align="left" width="100px;" height="30px;">用户是否注销</td>
					<td align="left" width="100px;" height="30px;">用户类型</td>
				</tr>
				<s:iterator value="pageBean.list" var="l">
					<tr onclick="selected(this)"
						...
						<td align="left" width="100px;" height="30px;">
						<s:property value="#this.password" /></td>						
						<td align="left" width="100px;" height="30px;">
						<s:select list="#{0:'用户已注销',1:'用户未注销'}" value="#this.flag"></s:select>
						</td>
						<td align="left" width="100px;" height="30px;">
						<s:property value="#this.type" /></td>
					</tr>
				</s:iterator>
			</table>
		...
		<script type="text/javascript">
			...
			function removet(){ 
				...
				formName.action="deleteManager.action"; 
				formName.submit();  
				}
			function search(){ 
				formName =document.forms[0];
			...
			function release(){
				window.location.href="<%=path%>/admin/addManager.action";
				}
			function modify(){ 
				if(selectedId==null)
				{
				...
				window.location.href="<%=path%>/admin/findManagerById.action?mid="+selectedId;
				} 
			/* 是否全选标记 */
